...
async function runIntegrityChecks() {
  ...
  if (isWin) {
    tasks.push(
      // wmic was removed in Windows 11 24H2 (and is deprecated since 21H1),
      // so it returns nothing on current installs and this VM check went
      // silently dark. Get-CimInstance is the supported replacement and is
      // present on every Windows 10+ box. Format-List output ("Manufacturer
      // : VMware, Inc.") lowercases into the same keyword match.
      _exec('powershell -NoProfile -NonInteractive -Command "Get-CimInstance Win32_ComputerSystem | Select-Object Manufacturer,Model | Format-List"', 8000).then(output => {
        if (!output) return;
  ...
  if (isMac) {
    ...
    tasks.push(
      // Apple's own "am I running under a hypervisor" flag (macOS 12+, both
      // Intel and Apple Silicon). The CPU-brand check above only catches a
      // QEMU/"virtual" brand string — but a Parallels / VMware Fusion / UTM
      // guest reports the HOST's real CPU brand, so those slipped through and
      // left Mac VM detection far behind the Windows Get-CimInstance
      // Manufacturer/Model check. kern.hv_vmm_present=1 catches every
      // hypervisor guest. Absent on macOS 11 and earlier → _exec returns ''
      // and this is skipped; bare metal reads '0' (no false positive).
      _exec('sysctl -n kern.hv_vmm_present', 3000).then(output => {
        if (output && output.trim() === '1') {
          flags.push({ type: 'vm_detected', severity: 'high',
            details: 'Running under a hypervisor (kern.hv_vmm_present=1)' });
        }
      })
    );
    tasks.push(
