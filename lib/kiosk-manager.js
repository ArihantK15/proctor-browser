...
// Best-effort disable of the Launchpad F4/app-switching surface — see
// the "UPDATE 2026-07-04" comment further down (createExamWindow's mac
// hardening block) for the research this is based on. F4/Launchpad is
// macOS's `com.apple.symbolichotkeys` entry 160, applied live via the
// real system utility `activateSettings -u` (no logout needed).
//
// CRITICAL, learned the hard way during live verification 2026-07-04:
// only ever touch key "160" specifically via `-dict-add`, NEVER
// `defaults delete com.apple.symbolichotkeys AppleSymbolicHotKeys` — that
// deletes the ENTIRE domain, wiping any of the user's own custom
// keyboard-shortcut bindings, not just this one entry. Restoring means
// writing `enabled = 1` back into key 160 specifically (Apple's own
// default state for this hotkey), never touching anything else in the
// dict. State is checked via PlistBuddy querying the single nested key
// directly (`:AppleSymbolicHotKeys:160:enabled`), not by parsing the
// whole dict's text output.
const _SYMBOLICHOTKEYS_PLIST = path.join(os.homedir(), 'Library', 'Preferences', 'com.apple.symbolichotkeys.plist');
const _ACTIVATE_SETTINGS = '/System/Library/PrivateFrameworks/SystemAdministration.framework/Resources/activateSettings';
function _setMacLaunchpadDisabled(disabled) {
  if (process.platform !== 'darwin') return;
  try {
    const { execFileSync } = require('child_process');
    if (disabled) {
      execFileSync('defaults', ['write', 'com.apple.symbolichotkeys', 'AppleSymbolicHotKeys', '-dict-add', '160',
        "{enabled = 0; value = { parameters = (65535, 131, 0); type = 'standard'; }; }"], { stdio: 'ignore' });
    } else {
      // Only restore if we're the ones who disabled it — a normal
      // launch/exit where it was never touched causes zero disruption.
      let current = '';
      try {
        current = execFileSync('/usr/libexec/PlistBuddy',
          ['-c', 'Print :AppleSymbolicHotKeys:160:enabled', _SYMBOLICHOTKEYS_PLIST],
          { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
      } catch(e) { return; } // entry not set — nothing to restore
      if (current !== '0' && current.toLowerCase() !== 'false') return; // already enabled
      execFileSync('defaults', ['write', 'com.apple.symbolichotkeys', 'AppleSymbolicHotKeys', '-dict-add', '160',
        "{enabled = 1; value = { parameters = (65535, 131, 0); type = 'standard'; }; }"], { stdio: 'ignore' });
    }
    execFileSync(_ACTIVATE_SETTINGS, ['-u'], { stdio: 'ignore' });
  } catch(e) {
    console.debug('[Kiosk] mac Launchpad %s failed: %s', disabled ? 'disable' : 'restore', e.message);
  }
}

// ── Mission Control + Launchpad watchdog (crash-safety net) ─────────
...
// this app process is even alive. Extended 2026-07-04 to also restore
// Launchpad hotkey 160 (same authorization, same shape). Justification:
// _setMacMissionControlDisabled/_setMacLaunchpadDisabled's restore calls
// only run from THIS process's own JS (releaseKiosk,
...
function _armMissionControlWatchdog() {
  ...
  try {
    ...
    // it fires WATCHDOG_SECS after being loaded, not immediately. Also
    // restores Launchpad hotkey 160 — writes enabled=1 back into that ONE
    // key specifically (never deletes the whole symbolichotkeys domain,
    // see _setMacLaunchpadDisabled's own comment on why that matters).
    const script = `defaults delete com.apple.dock mcx-expose-disabled 2>/dev/null; killall Dock 2>/dev/null; defaults write com.apple.symbolichotkeys AppleSymbolicHotKeys -dict-add 160 "{enabled = 1; value = { parameters = (65535, 131, 0); type = 'standard'; }; }" 2>/dev/null; ${_ACTIVATE_SETTINGS} -u 2>/dev/null; rm -f '${plist}'; launchctl bootout gui/$(id -u)/${_WATCHDOG_LABEL} 2>/dev/null`;
...
function createExamWindow() {
  ...
  const _armExamLockdown = () => {
    ...
    if (process.platform === 'darwin') {
      ...
      // activateSettings applies live, no Dock restart needed — order
      // relative to the Mission Control call above doesn't matter.
      _setMacLaunchpadDisabled(true);
...
function releaseKiosk({ reopenLobby = true } = {}) {
  ...
  if (process.platform === 'darwin') {
    ...
    _setMacLaunchpadDisabled(false);
...
module.exports = {
  ...
  setMacLaunchpadDisabled: _setMacLaunchpadDisabled,
