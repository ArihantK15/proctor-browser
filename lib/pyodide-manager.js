 ...
 *
 * @param {string} root  Absolute cache directory path.
 * @param {string} url   Full procta-pyodide:// URL string.
 * @returns {string}
 */
function resolvePyodideRequest(root, url) {
  // Reject ".." segments in the raw URL string before the URL constructor
  // normalises them away. This catches both literal and percent-encoded
  // traversal attempts (%2e%2e, %2E%2E, etc.).
  const rawDecoded = decodeURIComponent(url);
  if (/(^|[/\\])\.\.([/\\]|$)/.test(rawDecoded)) {
    throw new Error('pyodide path traversal attempt in URL: ' + url);
  }
  const u = new URL(url);
  const rel = decodeURIComponent(u.pathname.replace(/^\/+/, ''));
  const abs = path.normalize(path.join(root, rel));
  // Belt-and-suspenders: normalised path must still stay inside the root.
