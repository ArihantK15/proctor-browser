...
# ─── CONFIG ───────────────────────────────────────────────────────────────────
SERVER_URL = os.getenv("PROCTOR_SERVER_URL", "https://friendship-basename-rogers-transformation.trycloudflare.com/event")
SESSION_ID = os.getenv("PROCTOR_SESSION_ID", str(uuid.uuid4()))
...
def is_valid_phone(x1, y1, x2, y2):
    fw = max(x2-x1, 1); fh = max(y2-y1, 1)
    return (fh/fw) >= 1.2 and (fw*fh)/(W*H) <= 0.25
...
def run_yolo_fullframe(frame):
    ...
    for box in res.boxes:
        ...
        if cls_id in CHEAT_OBJECTS:
            ...
            if cls_id == 67 and not is_valid_phone(x1,y1,x2,y2):
                continue
