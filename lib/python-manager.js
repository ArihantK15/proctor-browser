...
async function downloadAudioModels() {
  // Resolves the script via the same dual-path lookup we use for
  // proctor.py — works in dev (./scripts/) and packaged
  // (process.resourcesPath/scripts/).
  const candidates = [
    path.join(__dirname, '..', 'scripts', 'download_audio_models.sh'),
    path.join(process.resourcesPath || '', 'scripts', 'download_audio_models.sh'),
  ];
  ...
  await new Promise(resolve => {
    // 5-minute total cap. The script itself runs fast on a cache hit
    // (no network) — the timeout is for the cold-start ~80 MB pull.
    const child = spawn('bash', [script], { timeout: 300000, stdio: 'ignore' });
    child.on('exit', code => {
