...
function _archiveNameAndChecksum() {
  ...
  if (process.platform === 'darwin') {
    ...
    return {
      name: `procta-runtime-mac-${isArm ? 'arm64' : 'x64'}.tar.gz`,
      checksum: isArm ? RUNTIME_ASSET_CHECKSUM_MAC_ARM64 : RUNTIME_ASSET_CHECKSUM_MAC_X64,
    };
  }
...
async function ensureRuntimeAssets(sendStatus, sendProgress) {
  ...
  //
  // Deliberately a SIBLING of cacheDir() rather than os.tmpdir(): the
  // rename() below must land on the same filesystem/volume as cacheDir(),
  // or it throws EXDEV (cross-device link). os.tmpdir() and the userData
  // parent dir CAN be different volumes (roaming/managed Windows profiles,
  // network home directories), which would otherwise fail every install on
  // those machines even though nothing is actually wrong.
  const extractTmp = path.join(path.dirname(dir), `.procta-runtime-extract-${Date.now()}`);
