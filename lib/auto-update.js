...
function initAutoUpdater(lobbyWindow, getMainWindow) {
  ...
  if (!ipcMain._proctaUpdateHandlerInstalled) {
    ...
    if (lobbyWindow && !lobbyWindow.isDestroyed()) {
      lobbyWindow.webContents.on('console-message', (...args) => {
        // Electron changed this event's signature: older builds pass
        // (event, level, message, line, sourceId); recent builds pass a
        // single Details object { message, level, ... }. Read both shapes
        // so the in-banner "Restart now" bridge keeps working across
        // versions (it silently no-op'd when the object form landed).
        const a0 = args[0];
        const msg = (a0 && typeof a0 === 'object' && 'message' in a0) ? a0.message : args[2];
  ...
  const _check = () => autoUpdater.checkForUpdates().catch(err => {
  ...
  // version on its own.
  _check();
  const _timer = setInterval(_check, 30 * 60 * 1000);
