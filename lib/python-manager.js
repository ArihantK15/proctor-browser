...
// (incl. torch) install into that venv exactly like the Windows flow.
...
async function runSetup() {
  ...
  if (process.platform === 'darwin') {
    // macOS: never pip into the system python3 (PEP 668) and never
    // write inside the signed .app. Use the bundled relocatable
...
async function startPython(sessionId, serverUrl, studentToken, calBiases) {
  ...
  if (!python) { console.error('[AI] No Python found — AI proctoring disabled'); pythonStarting = false; return; }
  if (!fs.existsSync(script)) { console.error('[AI] proctor.py not found'); pythonStarting = false; return; }
...
function stopPython() {
  ...
  if (pythonProcess) {
    // Capture the doomed process in a local so the SIGTERM fallback below
    ...
    // then silently SIGTERM the new process.
    const dying = pythonProcess;
    pythonProcess = null;
    try {
      if (process.platform === 'win32') {
        spawnSync('taskkill', ['/pid', String(dying.pid), '/f', '/t'],
          { timeout: 5000 });
      } else {
        // Send SIGINT first so Python can run its finally blocks (cap.release)
        dying.kill('SIGINT');
        // Fallback: SIGTERM if THIS process is still alive after 2s.
        const fallbackTimer = setTimeout(() => {
          try { dying.kill('SIGTERM'); } catch(e) {}
        }, 2000);
        fallbackTimer.unref();
      }
