...
const { ensureRuntimeAssets, cacheDir } = require('./runtime-assets.js');
...
function _pidFilePath() {
  try { return path.join(app.getPath('userData'), 'proctor.pids'); }
    ...
    // Fallback only when userData isn't resolvable (very early startup). Use a
    // per-user 0700 subdir, NOT a predictable world-writable temp name another
    // user on a shared box could pre-create or symlink-attack
    // (CodeQL js/insecure-temporary-file).
    const uid = (typeof process.getuid === 'function') ? process.getuid() : 'user';
    const dir = path.join(os.tmpdir(), `procta-${uid}`);
    try { fs.mkdirSync(dir, { recursive: true, mode: 0o700 }); } catch(_) {}
    return path.join(dir, 'proctor.pids');
  }
}
...
function _unrecordPid(pid) {
  ...
  // Read directly (no existsSync pre-check) to close the check-then-use race
  // (CodeQL js/file-system-race); a missing file just means nothing to prune.
  let content;
  try { content = fs.readFileSync(fp, 'utf8'); }
  catch(e) {
    if (e.code !== 'ENOENT') console.warn('[AI] pidfile prune read failed:', e.message);
    return;
  }
  try {
  ...
  } catch(e) { console.warn('[AI] pidfile prune write failed:', e.message); }
}

// Persist proctor stdout/stderr to a log file. The main-process console
// ([AI]/[CAL] lines below) isn't captured in a packaged build, so on-device
// detector diagnostics — [YOLO] Loading/Ready/<error>, the proctoring tier,
// frame-loop errors — were lost, making a real model-load failure invisible.
// Reuses the same logs dir as crash.log (main.js). Lazy single append stream;
// `false` sentinel = open failed, stop retrying. Must never break proctoring.
let _proctorLogStream = null;
function _proctorLog(line) {
  try {
    if (_proctorLogStream === null) {
      const dir = app.getPath('logs');
      try { fs.mkdirSync(dir, { recursive: true }); } catch(_) {}
      _proctorLogStream = fs.createWriteStream(path.join(dir, 'proctor.log'), { flags: 'a' });
      _proctorLogStream.on('error', () => { _proctorLogStream = false; });
    }
    if (_proctorLogStream) {
      _proctorLogStream.write(`${new Date().toISOString()} ${line}\n`);
    }
  } catch(_) { /* logging must never take down the proctor */ }
}
...
async function reapOrphanProctors() {
  ...
  let pids = [];
  try {
    // Read directly (no access() pre-check) to close the check-then-use race
    // (CodeQL js/file-system-race); ENOENT just means nothing to reap.
    const raw = await fs.promises.readFile(fp, 'utf8');
    pids = raw.split('\n')
      .map(s => parseInt(s.trim(), 10))
      .filter(n => Number.isInteger(n) && n > 0 && n !== process.pid);
  } catch(e) {
    if (e.code !== 'ENOENT') console.warn('[AI] reaper: pidfile read failed:', e.message);
    return;
  }
...
// Screen Recording TCC permission (macOS only), added 2026-07-04 for the
// screen-capture-while-blurred evidence feature (kiosk-manager.js). Two
// real differences from camera/mic that make this check need its own
// logic, not just a copy-paste of ensureCameraAccess:
//   1. There is NO askForMediaAccess('screen') — Apple only allows this
//      permission to be granted by the user manually, in System Settings.
//      We can never auto-prompt; not-determined and denied are both a
//      dead end from here, just phrased differently for the UI.
//   2. Critically: granting it in System Settings does NOT take effect
//      for an ALREADY-RUNNING process — confirmed by research (this is
//      documented, by-design macOS behavior, not a bug) — the app must
//      fully quit and relaunch. getMediaAccessStatus('screen') reads the
//      live TCC database and WILL report 'granted' immediately after the
//      user toggles it, even though this same running process still
//      can't actually capture anything until it restarts. Trusting the
//      status string alone would tell a student "you're all set" when
//      captures would silently fail for their entire exam. So this does
//      a REAL desktopCapturer.getSources() probe (not just the status
//      read) to distinguish "granted and working right now" from
//      "granted in Settings but this process needs a restart to use it."
//
// Windows note: Windows 11 has its own "Screenshots and screen recording"
// toggle (Settings > Privacy & security > App permissions), but it's
// genuinely unclear from available research whether that gate applies to
// traditional desktop/Win32 apps like Electron the same way it applies to
// UWP/Store apps — no Windows hardware in this environment to verify
// either way. Rather than assume Windows is unrestricted, this runs the
// SAME real-capture probe there too — if that Windows setting (or
// anything else) actually blocks desktopCapturer, this correctly detects
// it instead of wrongly reporting green.
async function ensureScreenRecordingAccess() {
  let systemPreferences, desktopCapturer;
  try { ({ systemPreferences, desktopCapturer } = require('electron')); }
  catch(e) { return { ok: true, status: 'unknown' }; }
  try {
    if (process.platform === 'darwin') {
      const status = systemPreferences.getMediaAccessStatus('screen');
      if (status !== 'granted') {
        // Includes 'not-determined' — there's no prompt to trigger here,
        // it just means the user has never opened the System Settings
        // pane for this. Same UI treatment as a plain denial.
        return { ok: false, status };
      }
    }
    // mac: status says granted — verify a real capture actually works
    // right now, to catch the restart-required case. Windows/Linux: no
    // meaningful status API, so this IS the whole check.
    try {
      const sources = await desktopCapturer.getSources({
        types: ['screen'],
        thumbnailSize: { width: 32, height: 20 }, // tiny — this is a capability probe, not real evidence
      });
      if (sources && sources.length) return { ok: true, status: 'granted' };
      return { ok: false, status: process.platform === 'darwin' ? 'granted-needs-restart' : 'denied' };
    } catch(e) {
      return { ok: false, status: process.platform === 'darwin' ? 'granted-needs-restart' : 'denied' };
    }
  } catch(e) {
    console.warn('[ScreenRecording] preflight failed:', e.message);
    return { ok: true, status: 'unknown' };
  }
}

// SECURITY: runs `cmd` through a SHELL. Only ever pass HARDCODED command
// strings (today: `${literal} --version` from the fixed python-name allowlist
// in resolvePython). NEVER interpolate a discovered/config/user path here — that
// would be shell command injection. For variable paths use spawn(file, [args])
// (no shell), as the rest of this module does.
...
// Path to the relocatable interpreter. As of the 2026-07 installer
// hardening this is no longer bundled inside the packaged app — it's
// fetched once by ensureRuntimeAssets() (lib/runtime-assets.js) into a
// per-user cache dir that survives app-code-only updates. Falls back to
// the OLD bundled-in-installer path for one transition release, so a
// machine mid-update (old bundled python-runtime/ still on disk, new
// cache not populated yet) doesn't break before the user's next full
// reinstall. Returns null if neither location has it (dev builds ship an
// empty python-runtime/ placeholder).
...
// pip dist names whose import name differs. Everything else imports under its
// own name ('-' → '_'). Lets the readiness probe DERIVE its imports from
// PIP_PACKAGES so the two can't drift — the old hardcoded list had to be
// hand-synced, exactly the kind of gap that left a half-installed env green.
const _PIP_IMPORT_OVERRIDES = {
  'opencv-python': 'cv2',
  'websocket-client': 'websocket',
};
function _pipToImport(spec) {
  const name = String(spec).split(/[<>=!~ ;]/)[0].trim();
  return _PIP_IMPORT_OVERRIDES[name] || name.replace(/-/g, '_');
}

// Critical bundled model weights — checked for DIAGNOSTICS only, never blocks
// launch (proctor.py degrades per-detector and the boot snapshot reports the
// real weights_missing). The readiness marker proves only that PACKAGES import,
// so a corrupted bundle would otherwise be a silent green. YOLO accepts either
// the YOLO26 export or the legacy v8 fallback.
function _warnMissingBundledWeights() {
  try {
    const dir = bundledWeightsDir();
    const hasYolo = ['yolo26n.onnx', 'yolov8n.onnx'].some(f => {
      try { return fs.statSync(path.join(dir, f)).size > 0; } catch (_) { return false; }
    });
    if (!hasYolo) {
      console.warn('[Setup] bundled YOLO weight missing (weights/yolo26n.onnx) ' +
        '— object detection will be unavailable this session.');
    }
  } catch (_) { /* diagnostics only — must never break launch */ }
}
...
async function checkPackagesReadyCached(python) {
  if (isPackagesMarkedReady(python)) {
    // The marker only covers package imports — surface a missing critical
    // weight here so a corrupted bundle isn't an invisible false-green.
    _warnMissingBundledWeights();
    return true;
  }
...
async function checkPackagesReady(python) {
  // Full import-readiness probe. The import list is DERIVED from PIP_PACKAGES
  // (via _pipToImport) so it can never drift from what the setup flow installs
  // — every package the proctor needs is imported once in a single subprocess.
  // Atomic: any missing package → false → the setup flow runs. The four-package
  // check this replaced passed silently when a half-completed install had cv2 +
  // uniface but not vosk, leaving audio + object detection disabled with no
  // signal. Object detection runs on onnxruntime (bundled weights/yolo26n.onnx,
  // legacy yolov8n.onnx fallback) — no ultralytics/torch — covered by the
  // derived `import onnxruntime`.
  return new Promise(resolve => {
    const importLine = PIP_PACKAGES.map(_pipToImport)
      .map(m => `import ${m}`).join('; ');
...
function downloadFile(url, dest, _redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req  = https.get(url, res => {
      // Follow redirects (301/302/303/307/308), bounded so a redirect loop
      // can't pin CPU/sockets. For our GETs all five are equivalent. Resolve
      // the Location against the current URL so relative redirects work.
      if ([301, 302, 303, 307, 308].includes(res.statusCode)) {
        ...
        if (!loc || _redirects >= 5) {
          reject(new Error(`Download failed: ${loc ? 'too many redirects' : 'redirect with no Location'} (HTTP ${res.statusCode})`));
          return;
        }
        downloadFile(new URL(loc, url).toString(), dest, _redirects + 1).then(resolve, reject);
        return;
      }
...
async function runSetup() {
  // Fetch/verify the runtime assets (base Python interpreter + weights/)
  // BEFORE anything below reads them — getBundledPython()/
  // bundledWeightsDir() (lib/python-manager.js) now read from the
  // per-user cache lib/runtime-assets.js populates, and this call is what
  // fills that cache on first run (or when RUNTIME_ASSET_VERSION bumps).
  // Deliberately NOT caught-and-ignored: a proctor that starts without
  // weights/the interpreter would crash-loop later with a confusing
  // error, so a failed fetch here must abort setup, not limp forward.
  try {
...
async function startSetupInBackground(getLobbyWin) {
  ...
  try {
    ...
    // Fast path requires BOTH package readiness AND a current runtime-asset
    // cache (weights/interpreter, see lib/runtime-assets.js). Packages and
    // runtime assets version independently — PIP_PACKAGES can be unchanged
    // while RUNTIME_ASSET_VERSION bumps (e.g. new model weights), and
    // checkPackagesReadyCached alone can't see that. Skipping this check
    // would silently strand an existing install on stale weights forever,
    // since runSetup() (the only thing that calls ensureRuntimeAssets) would
    // never run. If either signal is stale, fall through to the full
    // runSetup() path below, which re-verifies both.
    if (python && await checkPackagesReadyCached(python) &&
        !needsRuntimeAssetFetch(RUNTIME_ASSET_VERSION, cacheDir())) {
      _setSetup({ phase: 'ready', ready: true, label: 'Ready', pct: 100 }, getLobbyWin);
      return;
    }
...
// The weights dir. As of the 2026-07 installer hardening this is no
// longer bundled inside the packaged app (<resources>/weights) — it's
// fetched once by ensureRuntimeAssets() into a per-user cache dir that
// survives app-code-only updates (the object-detection weights + Vosk
// en/hi + Silero VAD models all live here now). Falls back to the OLD
// bundled-in-installer path for one transition release, so a machine
// mid-update (old bundled weights/ still on disk, new cache not
// populated yet) doesn't break before the user's next full reinstall. In
// dev, neither location typically has it, so the userData copy from a
// first-run download (audioWeightsDir()) is used instead — unchanged.
...
// proctor.py's model loaders (RetinaFace/SCRFD/YOLO/gaze) read the weights
// location via env vars — but split across TWO different names/semantics
// with no JS caller ever setting either: PROCTA_WEIGHTS_DIR (used directly
// as the weights dir) and ELECTRON_RESOURCES_PATH (the weights dir's
// PARENT — proctor.py appends "weights" itself). Neither var ever needed
// to be set before the runtime-assets decoupling, because proctor.py's
// fallback candidate (<script-dir>/weights/) worked automatically — both
// files shipped side-by-side under the old bundled-in-installer scheme.
// Now that weights/ lives in a per-user cache dir instead (see
// bundledWeightsDir() above), that fallback no longer matches anything on
// a fresh install, and with no env var set either, EVERY model loader
// silently fails to find its weights — face detection, gaze, and object
// detection all degrade to "unavailable" (found via a real Windows
// clean-install test: System Check reported "Minimal proctoring — face
// detection unavailable" despite Python + packages being fully ready).
// Every proctor.py spawn site must set BOTH names to the SAME resolved
// directory so every loader resolves correctly regardless of which env
// var name it happens to check.
function _weightsEnvVars() {
  const dir = bundledWeightsDir();
  return {
    PROCTA_WEIGHTS_DIR: dir,
    ELECTRON_RESOURCES_PATH: path.dirname(dir),
  };
}
...
const AUDIO_MODEL_NAMES = [
  ...
  // vosk Hindi dropped (~40 MB, not required for now). Kept OUT of this list so
  // audioModelsPresent() doesn't require it — English keyword spotting stays
  // "ready" with just en + silero, and audio_processor skips Hindi gracefully.
  'silero_vad.onnx',
];
...
async function startPython(sessionId, serverUrl, studentToken, calBiases) {
  ...
  const envVars = {
    ...
    PYTHONUTF8:                      '1',
    // Unbuffered stdout/stderr. Without this, a fast exit (e.g. the camera
    // open-failure sys.exit(1) path) loses everything still sitting in the
    // block buffer — the daemon dies with "Exited: 1" and a totally empty
    // log, making the actual failure undiagnosable. -u / PYTHONUNBUFFERED
    // flushes each line as it's printed so the real error reaches proctor.log.
    PYTHONUNBUFFERED:                '1',
    // Always headless: Electron is the only UI. proctor.py's HEADLESS default
    // is True on Windows but FALSE on macOS/Linux unless this is set, so
    // without it the main exam proctor pops cv2.imshow("AI Proctor") preview
    // windows over the student's exam on a Mac (Windows never showed them).
    // startCalibration already sets this; the main spawn must too.
    PROCTOR_HEADLESS:                '1',
    ...
    // Tells proctor.py's face/gaze/object-detection loaders where the
    // decoupled weights/ cache lives — see _weightsEnvVars()'s comment.
    ..._weightsEnvVars(),
  };
  ...
  proc.stdout.on('data', d => { const t = d.toString().trimEnd(); _proctorLog('[AI] ' + t); console.log('[AI]', t.trim()); });
  proc.stderr.on('data', d => { const t = d.toString().trimEnd(); _proctorLog('[AI:err] ' + t); console.error('[AI]', t.trim()); });
  proc.on('close', code => {
    _proctorLog('[AI] Exited: ' + code);
    ...
    if (code !== 0 && pythonShouldRun) {
      ...
      if (giveUp) {
        ...
        _proctorFailedNotify(
          'Proctoring could not start after several attempts. Close any app ' +
          'using your camera, restart your computer if you recently declined ' +
          'a Windows setup prompt during install, then restart the exam.');
        return;
      }
...
async function startCalibration(sessionId, serverUrl, studentToken, mainWindow) {
  ...
  const proc = spawn(python, [script], {
    ...
    env: {
      ...
      PROCTOR_SKIP_ENROLLMENT:  '1',
      PROCTOR_HEADLESS:         '1',
      // Calibration needs gaze/face detection loaded just like the main
      // proctor spawn — see _weightsEnvVars()'s comment.
      ..._weightsEnvVars(),
    }
  });
  calProcess = proc;
  ...
  proc.stderr.on('data', d => {
    ...
    if (text) {
      ...
      _proctorLog('[CAL:err] ' + text);
    }
    console.error('[CAL]', text);
  });
  proc.on('close', code => {
    _proctorLog('[CAL] Exited: ' + code);
...
function runSelfTest(python, timeoutMs = 60000) {
  return new Promise(resolve => {
    ...
    const env = {
      ...
      // Without these the selftest's face/gaze/object-detection probes
      // report "unavailable" even when the weights ARE present — see
      // _weightsEnvVars()'s comment (this exact symptom is what surfaced
      // the bug: a real Windows clean install's System Check).
      ..._weightsEnvVars(),
    };
    let out = '';
    let done = false;
...
//   screenRecording : { ok, status } from ensureScreenRecordingAccess
//   at      : ISO timestamp
function _buildSystemCheckResult(inputs) {
  const { python, pkgOk, selftest, camera, mic, audioOk, screenRecording, at } = inputs;
  const components = {};
  ...
  // Screen Recording — OPTIONAL (screen-capture-while-blurred is
  // defense-in-depth on top of the core proctoring pipeline, not a
  // requirement for it), so this is a warn, never a hard gate, same
  // treatment as mic/audio. Three distinct states matter here, not just
  // granted/denied — see ensureScreenRecordingAccess's own comment for
  // why 'granted-needs-restart' is a real, separate case worth its own
  // message (Screen Recording is the one macOS permission that doesn't
  // take effect for an already-running process).
  const sr = screenRecording || { ok: true, status: 'unknown' };
  if (sr.status === 'granted-needs-restart') {
    components.screenRecording = { status: 'warn', detail: 'Screen Recording granted — quit and reopen Procta to finish enabling it' };
  } else if (sr.ok) {
    components.screenRecording = { status: 'green', detail: sr.status === 'unknown' ? 'Available' : 'Access granted' };
  } else {
    components.screenRecording = { status: 'warn', detail: 'Screen Recording not granted — evidence capture during a focus-loss attempt will be skipped' };
  }

  // Overall readiness: the exam can START as long as Python + packages +
  // camera are green. Model tier (reduced/minimal), mic, speech models,
  // and Screen Recording NEVER flip ok=false — graceful degradation
  // (Phase 1.6) keeps the exam running with whatever loaded; they
...
async function runSystemCheck() {
  ...
  // 7) Screen Recording — OPTIONAL (screen-capture-while-blurred evidence
  // feature). No prompt exists for this permission; the check itself
  // verifies a REAL capture, not just the status string, to correctly
  // catch "granted in Settings but this process needs a restart."
  let screenRecording = { ok: true, status: 'unknown' };
  try { screenRecording = await ensureScreenRecordingAccess(); } catch (e) { screenRecording = { ok: true, status: 'unknown' }; }

  return _buildSystemCheckResult({ python, pkgOk, selftest, camera, mic, audioOk, screenRecording, at });
}
...
module.exports = {
  ...
  ensureMicAccess, ensureScreenRecordingAccess,
  runSelfTest, runSystemCheck, _buildSystemCheckResult,
  _restartStormDecision, _scrubSecretsFromEnv,
  bundledWeightsDir, _weightsEnvVars,
};
