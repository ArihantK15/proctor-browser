...
async function runSystemCheck() {
  ...
  let tier = null;
  if (python && pkgOk) {
    const st = await runSelfTest(python);
    if (st.ok && st.report) {
      const r = st.report;
      ...
      components.models = { status: 'warn', detail: 'Could not run the model self-test', error: st.error || 'unknown' };
    }
  } else {
  ...
  let cam = { ok: true, status: 'unknown' };
  try { cam = await ensureCameraAccess(); } catch (e) { cam = { ok: true, status: 'unknown' }; }
  ...
  components.mic = mic.ok
    ? { status: 'green', detail: mic.status === 'unknown' ? 'Available' : 'Access granted' }
...
module.exports = {
  ...
  ensureMicAccess, runSelfTest, runSystemCheck,
};
