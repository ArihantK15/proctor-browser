...
### Decision (2026-07-09): Option A — accept the tradeoff, narrowly
This reverses a deliberate prior fix: `lib/python-manager.js:1112-1118` bundles `weights/` at build time specifically so a fresh install needs **zero first-launch download** ("fully offline + deterministic"), after an earlier first-run-download approach broke with a user-visible "could not download" failure. Explicitly re-accepting that tradeoff here, narrowly scoped: **only the first-ever install, or the rare occasion the model weights themselves change, requires internet to fetch weights** — every subsequent app-code-only update stays small and untouched by this. Justification: Procta already requires internet to function at all (it's an online exam platform — login, exam data, every feature needs connectivity), so requiring it once at first install doesn't introduce a new *class* of failure, only moves *when* an internet-dependency can bite. This must be built with the same rigor the prior fix implies was missing the first time — real retry/backoff, real integrity verification, and a real, honest failure state (not a silent partial install) — see Testing below and the implementation plan's task breakdown.
