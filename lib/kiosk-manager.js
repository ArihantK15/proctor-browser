...
function createLobbyWindow() {
  ...
  lobbyWindow.webContents.on('did-fail-load', (_, errorCode, errorDescription, validatedURL) => {
    ...
    // Full path (with %USERPROFILE%) goes to console for our debugging
    // but NOT into the on-screen dialog. Privacy: a screenshot of the
    // error dialog should never leak the user's Windows account name.
    console.error('[Lobby] load failed:', errorCode, errorDescription, validatedURL);
    clearTimeout(lobbyShowFallback);
    // Redact home folder (%USERPROFILE%) and resourcesPath from the
    // visible URL. Leave the relative path so we still know WHICH file
    // failed (e.g. "…/resources/app.asar/app/static/student.html").
    const _redact = (s) => {
      if (!s) return '';
      let out = String(s);
      try {
        ...
        // Also collapse a Users/<name>/AppData prefix in case homedir
        // resolves to something different in packaged context.
