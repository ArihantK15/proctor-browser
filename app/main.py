...
from .routers.admin_coding import router as admin_coding_router
...
if SENTRY_DSN:
    try:
        ...
        # The PII scrubber + safe defaults live in app/observability.py so the
        # RQ worker (worker.py) shares EXACTLY this redaction logic — see that
        # module for why a single source of truth matters on a proctoring host.
        from .observability import scrub_sentry_event, SAFE_SENTRY_KWARGS

        sentry_sdk.init(
            dsn=SENTRY_DSN,
            # send_default_pii / include_local_variables / max_request_body_size
            ...
            **SAFE_SENTRY_KWARGS,
            ...
            # Release tag — sourced from GIT_SHA injected by the deploy
            # workflow. Falls back to APP_VERSION (semver) so Sentry can group
            # events by build instead of bucketing everything as "no release".
            release=(
            ...
            # AsyncioIntegration captures unhandled exceptions in the lifespan
            # background loops (reminder, reaper, ttl-sweeper, reconciler,
            # proctor-health, room-frame cleanup) — fire-and-forget create_task
            # exceptions otherwise never reach Sentry.
            integrations=[StarletteIntegration(), FastApiIntegration(), AsyncioIntegration()],
