...
<body>
  <h2>Setting Up Procta</h2>
  <p>First-time setup. Takes a few minutes — leave this window open.</p>
  <div class="bar-wrap">
  ...
  <script>
    ...
    ipcRenderer.on('setup-status', (_, msg) => {
      ...
      current.textContent = msg.replace(/^\\s+/, '');
    });
    ipcRenderer.on('setup-progress', (_, p) => {
