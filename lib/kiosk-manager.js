...
function createLobbyWindow() {
  ...
  lobbyWindow.webContents.on('will-navigate', (e, url) => {
    try {
      const u = new URL(url);
      // Allow the lobby's own bundle scheme (it is served from
      // procta-lobby://lobby) alongside the server origin + data/about —
      // mirrors the exam window's guard. Without procta-lobby: a future
      // in-page navigation within the lobby would be wrongly blocked.
