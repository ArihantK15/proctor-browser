...
Bug history:
  • The student-facing writer (/api/id-verification) did not stamp exam_id
    into violations.details, so /api/v1/admin/pending-verifications?exam_id=X
...
class TestPendingVerificationsReader:
    ...
    def _stub_violations_chain(self, mock_table, rows):
    ...
    def _stub_exam_sessions_chain(self, mock_table, sessions):
    ...
    def _router(self, mock_table, viol_rows, sessions):
        ...
        def _route(name):
            ...
            if name == "violations":
                return self._stub_violations_chain(mock_table, viol_rows)
            return self._stub_exam_sessions_chain(mock_table, sessions)
        return _route
    ...
    def test_filter_returns_only_matching_exam(self, client, admin_headers):
        ...
        with patch.object(shared_supabase_mock(), "table") as mock_table, \
             patch("app.dependencies._get_teacher_by_id",
                          return_value={"id": "teacher-1", "email": "p@t.com"}):
            mock_table.side_effect = self._router(mock_table, rows, [])
            resp = client.get(
                "/api/v1/admin/pending-verifications?exam_id=exam-A",
    ...
    def test_legacy_row_without_stamped_eid_falls_back_to_session_lookup(
        ...
        with patch.object(shared_supabase_mock(), "table") as mock_table, \
             patch("app.dependencies._get_teacher_by_id",
                          return_value={"id": "teacher-1", "email": "p@t.com"}):
            mock_table.side_effect = self._router(mock_table, rows, legacy_sessions)
            resp = client.get(
                "/api/v1/admin/pending-verifications?exam_id=exam-A",
                headers=admin_headers,
            )
