...
class TestCodingE2E:
    ...
    async def test_authoring_write_path_atomic_create_then_replace(self):
        """Drive the REAL authoring write — admin_coding._persist_coding_question_atomic
        — against Postgres. This is the ONLY coverage of that raw asyncpg SQL (the
        unit tests mock the pool), so it catches column/cast errors. Verifies create,
        then a replace that rewrites the cases (delete+insert in one transaction), so
        a stale case can't survive a replace."""
        from app.routers.admin_coding import _persist_coding_question_atomic
        from app.services import secrets_crypto

        exam_id = "coding-e2e-exam-write"
        qid = "coding-e2e-qwrite"
        await _teacher(TID_A)
        await _exam(TID_A, exam_id)

        options = {"allowed_languages": ["python"], "marks": 7,
                   "marks_policy": "partial", "time_limit_ms": 4000,
                   "starter_code": {"python": "print()"}}
        cases = [
            {"idx": 0, "input": "1 2", "expected_output": "3",
             "visibility": "sample", "float_tolerance": None},
            {"idx": 1, "input": "4 5", "expected_output": "9",
             "visibility": "hidden", "float_tolerance": 0.01},
        ]
        await _persist_coding_question_atomic(
            TID_A, exam_id, qid, "Add two numbers", json.dumps(options), cases, replacing=False)

        qrows = (await async_table("questions")
                 .select("question_id,question_type,question,options")
                 .eq("teacher_id", TID_A).eq("question_id", qid).execute()).data
        assert len(qrows) == 1
        assert qrows[0]["question_type"] == "coding"
        assert qrows[0]["question"] == "Add two numbers"
        assert json.loads(qrows[0]["options"])["marks"] == 7

        tcs = (await async_table("coding_test_cases")
               .select("idx,visibility,expected_output,float_tolerance")
               .eq("teacher_id", TID_A).eq("question_id", qid).order("idx").execute()).data
        assert [t["visibility"] for t in tcs] == ["sample", "hidden"]
        assert secrets_crypto.decrypt(tcs[0]["expected_output"]) == "3"
        assert tcs[1]["float_tolerance"] == 0.01

        # Replace: new statement + a single hidden case. The old two cases must be
        # gone (the delete+insert rewrite committed atomically with the update).
        cases2 = [{"idx": 0, "input": "x", "expected_output": "y",
                   "visibility": "hidden", "float_tolerance": None}]
        await _persist_coding_question_atomic(
            TID_A, exam_id, qid, "Replaced statement", json.dumps(options), cases2, replacing=True)

        qrows2 = (await async_table("questions").select("question")
                  .eq("teacher_id", TID_A).eq("question_id", qid).execute()).data
        assert qrows2[0]["question"] == "Replaced statement"
        tcs2 = (await async_table("coding_test_cases")
                .select("idx,expected_output").eq("teacher_id", TID_A)
                .eq("question_id", qid).order("idx").execute()).data
        assert len(tcs2) == 1
        assert secrets_crypto.decrypt(tcs2[0]["expected_output"]) == "y"
