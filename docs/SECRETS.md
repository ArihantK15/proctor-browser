...
| `/etc/procta/secrets.env` | B2 backup credentials read by `backup_to_b2.sh` | chmod 600, root only |
...
| `SUPABASE_SERVICE_ROLE_KEY` | Legacy Supabase service-role auth | Supabase dashboard (only if `DATABASE_BACKEND=supabase`) | n/a if on native Postgres | Migrated off in 2026; remove from `.env` if `DATABASE_BACKEND=postgres` |
