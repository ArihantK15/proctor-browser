...
function createLobbyWindow() {
  ...
  // Tracks whether we've already tried the network fallback so we
  // don't loop did-fail-load → remote-load → did-fail-load forever.
  let _triedRemoteFallback = false;
  lobbyWindow.webContents.on('did-fail-load', (_, errorCode, errorDescription, validatedURL) => {
    ...
    // Network fallback. If the local student.html in app.asar is
    // missing or unreadable (Defender quarantined a file inside the
    // asar, the asar was extracted partially, etc.), try the
    // identical lobby HTML served from app.procta.net/static/student.html.
    // The preload script still attaches because webPreferences.preload
    // is a local file path — IPC handlers (validate-student, log-event,
    // panic-unlock, etc.) keep working over remote-origin DOM.
    if (!_triedRemoteFallback && (validatedURL || '').startsWith('file://')) {
      _triedRemoteFallback = true;
      const remoteUrl = (SERVER_URL || 'https://app.procta.net').replace(/\/$/, '') +
                        '/static/student.html';
      console.warn('[Lobby] local bundle failed — trying network fallback:', remoteUrl);
      clearTimeout(lobbyShowFallback);
      lobbyWindow.loadURL(remoteUrl).catch(e => {
        console.error('[Lobby] network fallback also failed:', e.message);
      });
      lobbyWindow.show();
      return;
    }

    // Both local AND network failed. Show the error dialog with a
    // redacted path so screenshots don't leak the user's account name.
    clearTimeout(lobbyShowFallback);
    const _redact = (s) => {
      if (!s) return '';
      let out = String(s);
