    ...
    *sub_row_before* is the subscription-row snapshot taken **before** the
    ...
    if net <= 0:
        ...
        try:
            ...
            if credit_consumed:
                try:
                    cur = await _atable("organizations")\
                        .select("billing_credit_inr").eq("id", oid).limit(1).execute()
                    cur_credit = int((cur.data[0].get("billing_credit_inr") if cur.data else 0) or 0)
                    await _atable("organizations").update(
                        {"billing_credit_inr": cur_credit + credit_consumed}
                    ).eq("id", oid).execute()
                    credit_consumed = 0
                except Exception as exc:
