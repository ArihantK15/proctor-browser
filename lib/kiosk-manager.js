...
function createExamWindow() {
  ...
  const _armExamLockdown = () => {
    ...
    // This watcher is the REAL defense against every OS-reserved switching
    // surface this audit found genuinely uninterceptable (Cmd+Tab, Cmd+
    // Space, Mission Control, Launchpad — see config.js and the removed-
    // fix comments above) — all of them steal key-window status from the
    // exam the instant they activate, which fires 'blur' regardless of
    // the specific cause. Detect + refocus fast is the same approach
    // competitor lockdown browsers almost certainly use too: no vendor
    // has published a way around the same OS-level protections, so
    // "blocks task switching" in their marketing is most plausibly this
    // same pattern, not a magic prevention this audit somehow missed.
    //
    // Added 2026-07-04: report every blur as a violation, not just a
    // silent refocus. Previously this yanked focus back with ZERO record
    // of the attempt — a teacher reviewing the exam afterward would have
    // no idea a student tried to switch away at all. Rate-limited to one
    // report per 2s so a rapid blur/focus flicker (if one ever happens)
    // can't flood the server, while still capturing separate attempts
    // spaced further apart (which matters for spotting a pattern).
    let _blurTimer = null;
    let _lastBlurReportAt = 0;
    mainWindow.on('blur', () => {
      if (!mainWindow || mainWindow.isDestroyed()) return;
      const now = Date.now();
      if (now - _lastBlurReportAt > 2000) {
        _lastBlurReportAt = now;
        const sid = currentSessionId;
        if (sid && studentToken) {
          (async () => {
            try {
              ...
              await fetch(`${SERVER_URL}/api/v1/event`, {
                ...
                signal: ac.signal,
                body: JSON.stringify({ session_id: sid,
                  event_type: 'exam_window_blur', severity: 'medium',
                  details: 'Exam window lost focus (app switch, Mission Control, Launchpad, Spotlight, or similar) — refocused automatically within 100ms.' }),
              });
              clearTimeout(timer);
