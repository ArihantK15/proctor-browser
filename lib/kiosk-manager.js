    ...
    mainWindow.on('blur', () => {
      ...
      _blurTimer = setTimeout(() => {
        ...
        if (mainWindow && !mainWindow.isDestroyed() && !mainWindow.isFocused()) {
          ...
          // (ForegroundLockTimeout) — NOT the same mechanism as macOS's,
          // and Electron's `steal` option is mac-only, confirmed no
          // equivalent exists for it. moveTop() is a cheap, harmless
          // addition some developers report helps win the z-order fight
          // there, but this is NOT independently verified — no Windows
          // hardware available in this environment to confirm it
          // actually overrides ForegroundLockTimeout the way `steal`
          // is confirmed to on mac. A properly verified Windows fix
          // would need either a native module (e.g.
          // node-windows-foreground-love, wraps AllowSetForegroundWindow)
          // or the AttachThreadInput win32 trick — not attempted blind.
          ...
          (async () => {
            try {
              ...
              await fetch(`${SERVER_URL}/api/v1/event`, {
                ...
                signal: ac.signal,
                body: JSON.stringify({ session_id: sid,
                  event_type: 'exam_window_blur', severity,
